  ...
  if (action == MenuAction_Select) {
    ...
    if (choice == -1) {
      ...
      if (IsPlayer(randomClient))
        C5_PUG_SetCaptain(1, randomClient, true);
    } else if (IsPlayer(choice)) {
      C5_PUG_SetCaptain(1, choice, true);
    }
...
public int Captain2MenuHandler(Menu menu, MenuAction action, int param1, int param2) {
  if (action == MenuAction_Select) {
    ...
    if (choice == -1) {
      ...
      if (IsPlayer(randomClient))
        C5_PUG_SetCaptain(2, randomClient, true);
    } else if (IsPlayer(choice)) {
      C5_PUG_SetCaptain(2, choice, true);
    }
  } else if (action == MenuAction_End) {
    CloseHandle(menu);
  }
