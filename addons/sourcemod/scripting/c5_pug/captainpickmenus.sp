...
public int InitialChoiceHandler(Menu menu, MenuAction action, int param1, int param2) {
  if (action == MenuAction_Select) {
    ...
    if (StrEqual(choice, "player")) {
      C5_MessageToAll("%T", "InitialPickPlayerChoice", client, captString);
      SideMenu(g_capt2);
    } else if (StrEqual(choice, "side")) {
      C5_MessageToAll("%T", "InitialPickSideChoice", client, captString);
      SideMenu(g_capt1);
    } else {
...
public int SideMenuHandler(Menu menu, MenuAction action, int param1, int param2) {
  if (action == MenuAction_Select) {
    ...
    C5_MessageToAll("%t", "SideChoiceSelected", captString, teamString);
  ...
  if (IsPickingFinished()) {
    ...
    if (IsValidClient(client)) {
      ...
      if (AddPlayersToMenu(menu) > 0) {
        ...
        C5_MessageToAll("Not enough players for picking, aborting the game.");
        EndMatch(false);
        delete menu;
      }
    } else {
      C5_MessageToAll("A captain is missing, aborting the game.");
      EndMatch(false);
    }
  }
...
public int PlayerMenuHandler(Menu menu, MenuAction action, int param1, int param2) {
  ...
  if (action == MenuAction_Select) {
    ...
    if (IsPlayer(selected)) {
      ...
      C5_MessageToAll("%t", "PlayerPickChoice", captName, selectedName);
    ...
    C5_MessageToAll("Failed to get captain pick. Aborting the game.");
    EndMatch(false);
