#define KNIFE_CONFIG "sourcemod/c5_pug/knife.cfg"
...
public int KnifeVoteMenuHandler(Menu menu, MenuAction action, int param1, int selection)
{
	switch(action)
	{
      ...
      if (StrEqual(info, "T"))
      {
        C5_MessageToAll("%T", "ChooseTeamT", LANG_SERVER, client);
        FakeClientCommand(client, "sm_t");
      }
      else if(StrEqual(info, "CT"))
      {
        C5_MessageToAll("%T", "ChooseTeamCT", LANG_SERVER, client);
...
public Action Command_Stay(int client, int args) {
  if (AwaitingDecision(client, "sm_stay")) {
    if (g_DoVoteForKnifeRoundDecisionCvar.IntValue == 0) {
      ...
      C5_Message(client, "%t", "KnifeRoundVoteStay");
...
public Action Command_Swap(int client, int args) {
  if (AwaitingDecision(client, "sm_swap")) {
    if (g_DoVoteForKnifeRoundDecisionCvar.IntValue == 0) {
      ...
      C5_Message(client, "%t", "KnifeRoundVoteSwap");
      HandleKnifeDecisionVote();
    }
  }
  return Plugin_Handled;
