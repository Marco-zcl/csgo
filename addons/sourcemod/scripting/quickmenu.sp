	...
	if(KvGotoFirstSubKey(kv))
	{
		...
		do
		{
			...
			else
			{
				...
				KvGetString(kv, "message", g_Menu[iTotalChoice].sMessage, 64);
				iTotalChoice++;
			}
		}
...
public int ShowQuickMenuHandler(Menu menu, MenuAction action, int client, int param2)
{
	switch(action)
	{
		case MenuAction_Select:
		{
			for (int i = 0; i < iTotalChoice; i++)
			{
				if(param2 == i)
				{
					ClientCommand(client, "%s", g_Menu[i].sCommand);
					PrintHintText(client, "%s", g_Menu[i].sMessage);
				}
			}
		}
		case MenuAction_End:
